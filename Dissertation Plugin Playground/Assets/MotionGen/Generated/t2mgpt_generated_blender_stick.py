...
JSON_PATH = Path(r"C:\Users\jack\OneDrive\Desktop\Coding\Dissertation\Dissertation Plugin Playground\Assets\MotionGen\Generated\t2mgpt_generated.json")
COLLECTION_NAME = "T2M_GPT_StickFigure"
JOINT_RADIUS = 0.035
BONE_RADIUS = 0.018
...
def load_motion(path: Path):
    ...
    for frame in frames:
        frame_positions = []
        joints_source = frame.get("worldJoints") or frame["joints"]
        for joint in joints_source:
            # The canonical JSON stores:
            # - joints: root-relative joint positions
            # - worldJoints: absolute/world-space joint positions
            # For Blender we need the absolute positions to match test_generate.py.
            # Blender is Z-up, so map (x, y, z) -> (x, z, y).
...
def main():
    ...
    print(f"Imported {len(frames)} frames from {JSON_PATH}")
